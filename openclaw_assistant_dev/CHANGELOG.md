...
### Fixed
- Fixed silent Homebrew installation failure by downloading install script first, then executing as linuxbrew user
- Added explicit verification that brew binary exists after installation
- Ensured proper PATH includes /usr/sbin for useradd command during installation
- Improved Dockerfile reliability with step-by-step Homebrew installation process

## [0.6.4] - 2026-04-11

### Fixed
- Fixed silent Homebrew installation failure by adding NONINTERACTIVE=1 and explicit environment sourcing.
...
### Changed
- Switch Dev-Addon to use official OpenClaw v2026.4.9 instead of chillkiller fork
- Add system dependencies for native modules: libvips-dev (sharp), liblapack3/libopenblas-dev (lancedb), libopus-dev/ffmpeg (opusscript/silk-wasm)
- Remove unnecessary fork build process - install directly from npm
- Keep all existing Dev-Addon features: Homebrew, Chromium, Proxy-Shim, ttyd, MCP auto-config, etc.
- This resolves instability in v4.9+ caused by missing native module dependencies
...
- DOCS: new **MCP Integration** guide covering automatic/manual setup, verification, model tips, and troubleshooting.
...
- In `lan_reverse_proxy` and other non-`lan_https` setups, Control UI origins also include the origin derived from `gateway_public_url`.
- Control UI origins configuration is now consistently applied via merge logic (defaults + existing values + user extras), reducing manual `openclaw.json` edits after upgrades.
...
- **`access_mode` preset option** — simplifies secure access configuration with one setting:
  ...
  - `lan_reverse_proxy`: LAN bind + trusted-proxy for an external reverse proxy (NPM, Caddy, Traefik)
...
- **Built-in TLS certificate generation** (`lan_https` mode):
  ...
  - Server certificate is regenerated automatically when LAN IP changes
...
### Changed
- Enhance README with images and updated setup instructions

---

For the full commit history, see [GitHub commits](https://github.com/chillkiller/OpenClawHomeAssistant/commits/main).