...
def patch_openclaw_config() -> None:
    """Ensure openclaw.json enables ACPX and defines coding agents/harnesses."""
    ...
    if acp.get("enabled") is not True:
        acp["enabled"] = True
        changed = True
    if acp.get("backend") != "acpx":
        acp["backend"] = "acpx"
        changed = True

    allowed = set(acp.get("allowedAgents", []))
    ...
    # 2. Ensure agent definitions
    agents_list = cfg.setdefault("agents", {}).setdefault("list", [])

    def upsert_agent(agent_id: str, harness: str, name: str, emoji: str) -> bool:
        for agent in agents_list:
            if agent.get("id") == agent_id:
                # Ensure harness is set and identity is consistent
                agent_changed = False
                ...
                if identity.get("emoji") != emoji:
                    identity["emoji"] = emoji
                    agent_changed = True
                # Ensure workspace is set for coding agents
                expected_workspace = f"/config/clawd/agents/{agent_id}"
                if agent.get("workspace") != expected_workspace:
                    agent["workspace"] = expected_workspace
                    agent_changed = True
                # Ensure subagent allowAgents includes the coding pair
                sub = agent.setdefault("subagents", {})
                allowed_subs = set(sub.get("allowAgents", []))
                coding_subs = {"coding-main", "coding-review"}
                if not coding_subs.issubset(allowed_subs):
                    sub["allowAgents"] = sorted(allowed_subs | coding_subs)
                    agent_changed = True
                # Tools profile full
        ...
        agents_list.append({
            "id": agent_id,
            "name": name,
            "workspace": f"/config/clawd/agents/{agent_id}",
            "agentDir": f"/config/clawd/agents/{agent_id}/.agent",
            "harness": harness,
            "identity": {"emoji": emoji},
            "subagents": {"allowAgents": sorted({"coding-main", "coding-review"})},
            "sandbox": {"mode": "off"},
            "tools": {"profile": "full"},
        })
        return True

    if upsert_agent("coding-main", "claude", "Forge", "🏗️"):
        changed = True
    if upsert_agent("coding-review", "codex", "Audit", "🔍"):
        changed = True

    # 3. Ensure harness definitions for legacy/declarative OpenClaw configs.
    #    These point to the wrapper launchers so OpenClaw can spawn them.
    harnesses = cfg.setdefault("harnesses", {})
    for harness_id, wrapper_name in (
        ("claude", "claude-agent-acp-wrapper.mjs"),
        ("codex", "codex-acp-wrapper.mjs"),
        ("opencode", "opencode-acp-wrapper.mjs"),
    ):
        harness = harnesses.setdefault(harness_id, {})
        wrapper_path = str(ACPX_DIR / wrapper_name)
        if harness.get("command") != wrapper_path:
            harness["command"] = wrapper_path
            changed = True
        if harness.get("type") != "acpx":
            harness["type"] = "acpx"
            changed = True
    ...
    if changed:
        try:
            write_json(config_path, cfg)
            log("Patched openclaw.json with ACPX harness/agent configuration")
        except Exception as e:
            log(f"ERROR: failed to write {config_path}: {e}")
    else:
        log("openclaw.json ACPX configuration already correct")
