...
let command;
let args;
if (configuredArgs[0] === "--openclaw-run-configured") {
  command = configuredArgs[1];
  args = configuredArgs.slice(2);
} else {
  command = opencodeCommand;
  args = ["acp", ...configuredArgs];
}

if (!command) {
  console.error("[openclaw] missing configured OpenCode ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(`[openclaw] failed to launch OpenCode ACP wrapper: ${error.message}`);
  process.exit(1);
});
